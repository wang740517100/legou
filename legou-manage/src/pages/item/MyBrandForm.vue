<template>
  <v-form v-model="valid" ref="BrandForm">
    <v-text-field v-model="brand.name" label="品牌名称" required :rules="[v => !!v || '品牌名称不能为空']"></v-text-field>
    ...
    <v-layout row>
      ...
      <v-flex>
        <v-upload
          ref="image_url"
          v-model="brand.image"
          url="/upload/image?access-token=1"
          :multiple="false"
          :pic-width="250"
          :pic-height="90"
        />
...
<script>
    export default {
      ...
      methods:{
        submit(){
          //提交表单
          if(this.$refs.BrandForm.validate()){ //表单验证
            /**
             * 使用解构表达式获取数据，除categories以外的数据都放入rest中，
             * 然后对categories使用map进行处理，得到id后重新赋值给
             * rest里面的categories数组
             */
            const {categories, ... rest} = this.brand;
            rest.categories = categories.map(c => c.id).join(",");
            if (this.isEdit) { //如果是编辑就先删除分类_品牌关系表
