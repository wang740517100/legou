<template>
  <v-card>
    <v-card-title>
      <v-btn color="primary" small @click="addGoods">新增商品</v-btn>
      <v-btn color="error" small @click="deleteAllGoods">全部删除</v-btn>
      <v-btn color="secondary" small @click="soldOutPutAll" v-if="filter.saleable === false">全部上架</v-btn>
      <v-btn color="secondary" small @click="soldOutPutAll" v-else>全部下架</v-btn>
      <v-spacer></v-spacer>
      <v-flex xs3>
        状态：
      ...
      methods:{
        ...
        editGoods(oldGoods){
          ...
          this.$http.get("/item/goods/spu/"+oldGoods.id).then(({data}) => {
            ...
            this.oldGoods.spuDetail.genericSpec = JSON.parse(data.spuDetail.genericSpec);
            this.oldGoods.spuDetail.specialSpec = JSON.parse(data.spuDetail.specialSpec);
            //specTemplate     genericSpec
            // specifications     specialSpec
